define(['kinetic', 'preload', 'js/assets', 'js/enemies'], function(k, p, assets, enemies) {
  ...
  game.init = function() {
    ...
    function main(event) {
      ...
      layers['loading'] = new Kinetic.Layer({
        x: (stage.getWidth() / 2) - (320 / 2),
        y: (stage.getHeight() / 2) - (285 / 2),
        width: 320,
        height: 285
      });

      layers['game'] = new Kinetic.Layer({
        x: 0,
        y: 0,
        width: SIZE * 20,
        height: SIZE * 10
      ...
      function initGame() {
        ...
        $(window).resize(function(e) {
          ...
          bg_rect.fillLinearGradientEndPoint({
            x: window.innerWidth,
      ...
      function Player() {
        ...
        this.head = new Kinetic.Image({
          x: 0,
          y: 0,
          width: this.sprint,
          height: this.sprint,
          image: assets_queue.getResult('snake_head')
        });
        this.actor = [];
        this.food = new Kinetic.Image({
          x: layers['game'].getWidth() / 2,
          y: layers['game'].getHeight() / 2,
          width: this.sprint,
          height: this.sprint,
          image: assets_queue.getResult(this.enemy['pic'])
        });
        ...
        this.update = function() {
          ...
          if (x2 == this.food.x() && y2 == this.food.y()) {
            ...
            var new_actor = new Kinetic.Image({
              x: x,
              y: y,
              width: this.sprint,
              height: this.sprint,
              image: assets_queue.getResult('snake_body')
            });
